...
public class RegulaOutput extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private TableLayout rTableLayout;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        setContentView(R.layout.activity_regula_output);
        Toolbar toolbar = findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        err_regula= findViewById(R.id.error_msg_regula);
        err_regula.setVisibility(View.GONE);
        rTableLayout = findViewById(R.id.tableRegula);
        rTableLayout.setStretchAllColumns(true);
        drawerLayout = findViewById(R.id.drawerRegula);
    ...
    public void loadDataBisection()
    {
        ...
        Regula_Method regula_methodObject= new Regula_Method(input_eqn);
        ArrayList<RegulaDType> list = regula_methodObject.regulaM(a,b);
        int rows= list.size();
        if(rows==0){
            getSupportActionBar().setTitle("Result");
            err_regula.setVisibility(View.VISIBLE);
            err_regula.setText("The given parameter does not enclose a root");
        }
            ...
            rTableLayout.removeAllViews();
            for (int i = -1; i < rows; i++) {
                RegulaDType row = null;
                if (i > -1) {
                    row = list.get(i);
                } else {
                ...
                rTableLayout.addView(tr, trParams);
                ...
                if (i > -1) {
                    ...
                    tvSep.setHeight(1);
                    trSep.addView(tvSep);
                    rTableLayout.addView(trSep, trParamsSep);
                }
            }
            //To be worked on how to print final result
            /*err_regula.setVisibility(View.VISIBLE);
            RegulaDType result = list.get(rows-1);
            double resultDouble = Double.parseDouble(result.parameterA);
            String resultFinal = decimalFormat.format(resultDouble);
            err_regula.setText(String.format("The root is: %s", resultFinal));*/
        }
    }
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        switch (id){
            ...
            case R.id.nav_home:
                Intent h= new Intent(RegulaOutput.this,MainActivity.class);
                h.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                startActivity(h);
                break;
            case R.id.nav_bisection:
                Intent i= new Intent(RegulaOutput.this, BisectionInput.class);
                i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                startActivity(i);
                break;
            case R.id.nav_newton:
                Intent g= new Intent(RegulaOutput.this, Newton_RaphsonInput.class);
            ...
            case R.id.nav_regula:
                Intent j = new Intent(RegulaOutput.this, RegulaInput.class);
